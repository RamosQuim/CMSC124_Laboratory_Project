...
def analyzetext():
    ...
    returned_value = sample.connect_UI(textEditor_Content)[0]
    returned_value1 = sample.connect_UI(textEditor_Content)[1]
    # print(f"returned_value: {returned_value}")
    lexResults.append(returned_value)
    symbolResults.append(returned_value1)
    # print(lexResults)


    #this part will show the newly added things!!
    for item in lexResults:
        # print(f"item:{item}")
        for j in item:
            # print(f"j:{j}")
